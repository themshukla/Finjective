...
function DroppableSection({ id, children }: { id: string; children: React.ReactNode }) {
  ...
  return (
    <div ref={setNodeRef} className={`min-h-[20px] rounded-lg transition-colors ${isOver ? "bg-primary/5" : ""}`}>
      {children}
    </div>
  );
}
      ...
      >
        ...
        <section className="mt-5">
          <div className="sticky top-0 z-10 rounded-xl bg-muted border border-border px-3 py-1.5 -mx-1.5">
            ...
            <div className="flex gap-3 mt-0.5">
              <span className="text-xs text-muted-foreground tabular-nums">${totalBudgetedExpenses.toLocaleString()} budgeted</span>
              <span className="text-xs text-muted-foreground tabular-nums">${totalExpenses.toLocaleString()} actual</span>
              <span className={`text-xs tabular-nums ${(totalBudgetedExpenses - totalExpenses) >= 0 ? "text-muted-foreground" : "text-expense"}`}>
          ...
          return (
            <section key={section.id} className="mt-5">
              <div className="sticky top-0 z-10 rounded-xl bg-muted border border-border px-3 py-1.5 -mx-1.5">
                ...
                <div className="flex gap-3 mt-0.5">
                  <span className="text-xs text-muted-foreground tabular-nums">${sectionBudgeted.toLocaleString()} budgeted</span>
                  <span className="text-xs text-muted-foreground tabular-nums">${sectionTotal.toLocaleString()} actual</span>
                  <span className={`text-xs tabular-nums ${(sectionBudgeted - sectionTotal) >= 0 ? "text-muted-foreground" : "text-expense"}`}>
